...
function createtime1() {
  var grt = new Date("11/17/2021 10:00:00"); //此处修改你的建站时间或者网站上线时间
...
if (m == 5 && dd == 22) {//程艾博生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝程艾博" + (y - 2001).toString() + "岁生日快乐！🐱");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 12 && dd == 18) {//卢凯威生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝傻逼卢凯威" + (y - 2002).toString() + "岁生日快乐！🐱");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  let currentTimeHtml = "";
  if (hnum >= 0 && hnum < 6) {
    currentTimeHtml = `<img class='boardsign' src='https://img.shields.io/badge/这个点了应该去睡觉啦，熬夜对身体不好哦~-6adea8?style=social&logo=coffeescript' title='这个点了应该去睡觉啦，熬夜对身体不好哦~'><br>`;
  } else if (hnum >= 6 && hnum < 9) {
    currentTimeHtml = `<img class='boardsign' src='https://img.shields.io/badge/起床啦！-6adea8?style=social&logo=coffeescript' title='早安！起床啦！'><br>`;
  } else if (hnum >= 9 && hnum < 20) {
    currentTimeHtml = `<img class='boardsign' src='https://img.shields.io/badge/本站正在营业中-6adea8?style=social&logo=cakephp' title='距离百年老站也就差不到一百年~'><br>`;
  } else if (hnum >= 20 && hnum < 24) {
    currentTimeHtml = `<img class='boardsign' src='https://img.shields.io/badge/下班休息啦！-6adea8?style=social&logo=coffeescript' title='下班休息啦！'><br>`;
  }
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 帧率监测 (不要关闭) </div><input type="checkbox" id="fpson" onclick="fpssw()">
